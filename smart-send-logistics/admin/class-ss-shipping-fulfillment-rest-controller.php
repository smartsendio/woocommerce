 ...
 * SS_Shipping_Delivery_Details::from_array(); the controller only ever
 * calls SS_Shipping_Fulfillment_Service::fulfill_outbound()/fulfill_return()
 ...
 *
 * @package  SS_Shipping_Fulfillment_Rest_Controller
 * @category Shipping
 * @author   Smart Send
 */

// A second copy of the plugin may already have defined the class.
if ( ! class_exists( 'SS_Shipping_Fulfillment_Rest_Controller' ) ) :

	class SS_Shipping_Fulfillment_Rest_Controller {
		 ...
		 * @param SS_Shipping_Pickup_Point_Lookup         $pickup_point_lookup Shared pickup point lookup.
		 ...
		 */
		public function __construct( SS_Shipping_Fulfillment_Service $fulfillment_service, SS_Shipping_Order_Fulfillment_Presenter $presenter, SS_Shipping_Shipment_Ids $shipment_ids, SS_Shipping_Method_Resolver $method_resolver, SS_Shipping_Pickup_Point_Lookup $pickup_point_lookup, SS_Shipping_Order_Meta $order_meta, ?SS_Shipping_Settings $settings = null ) {
		 ...
		 * SS_Shipping_Delivery_Details::from_array() property for property
		...
		public function fulfill( $request ) {
			...
			$details = SS_Shipping_Delivery_Details::from_array( (array) $request->get_param( 'delivery_details' ) );
		 ...
		 *
		 * @param SS_Shipping_Delivery_Details $details The submitted details (modified in place).
		 * @param WC_Order                     $order   The order.
		 * @param string                       $method  The method the flow books with ('' when unknown).
		 *
		 * @return true|WP_Error
		 */
		protected function resolve_pickup_point_override( SS_Shipping_Delivery_Details $details, WC_Order $order, string $method ) {
			...
			$method_code = new SS_Shipping_Method_Code( $method );
		 ...
		 *
		 * @param WC_Order                     $order     The order.
		 * @param SS_Shipping_Delivery_Details $details   The submitted details.
		 * @param boolean                      $is_return Which flow.
		 *
		 * @return string
		 */
		protected function method_for_flow( WC_Order $order, SS_Shipping_Delivery_Details $details, bool $is_return ): string {
		 ...
		 * @return void
		 */
		protected function fill_item_names( SS_Shipping_Delivery_Details $details, WC_Order $order ): void {
			...
			$named = new SS_Shipping_Parcel_Plan();
			foreach ( $plan->get_specs() as $spec ) {
				$copy = SS_Shipping_Parcel_Spec::from_array( array_merge( $spec->to_array(), array( 'items' => array() ) ) );
		 ...
		 * merchant can fix.
		 *
		 * @param SS_Shipping_Pickup_Point_Not_Found_Exception $e      The miss.
		 * @param integer                                      $status 404 on the lookup route, 422 on a booking request.
		 *
		 * @return WP_Error
		 */
		protected function pickup_point_not_found_error( SS_Shipping_Pickup_Point_Not_Found_Exception $e, int $status ): WP_Error {
			return new WP_Error(
