 ...
 * Shipment_Builder when it resolves a
 * Parcel_Plan (the input side, made of
 * Parcel_Spec) against the order's item lines. Lives on
 * Shipment, replacing the untyped parcel arrays it held
 * before.
 *
 ...
 *
 * Item rows stay plain arrays (see Order_Reader::
 * get_items_data() for the row shape) for the same reason
 * Shipment keeps its nested sections as arrays.
 *
 * Like Shipment, this class makes zero WordPress calls: it
 * is a pure data holder read by
 * \Smart_Send\API\Resources\Booking_Resource::from_shipment().
 */
class Parcel {
