 ...
 *
 * book() takes the WC_Order plus the Delivery_Details
 * fulfillment decided on (method, pickup point, parcel plan) and:
 *
 *   1. builds the request DTO, Shipment, through
 *      Shipment_Builder (#113) and runs the
 *      smart_send_booking_request filter on it;
 *   2. hands it to \Smart_Send\API\Resources\Booking_Resource (#112), which
 *      translates it into the v1 wire request and sends it;
 *   3. maps the response into the result DTO,
 *      Booked_Shipment, fires smart_send_booking_completed
 *      and returns it - or, when the API rejected the shipment, fires
 *      smart_send_booking_failed and throws
 *      Booking_Exception (the API exception as previous,
 ...
 * Fulfillment_Service's job, before book() is called.
 ...
 * Booked_Shipment: map_v1_response() is the one place
 ...
 * (Order_Reader, Shipment_Builder) are
...
class Booking_Service {
	 ...
	 *
	 * @throws Booking_Exception When the shipment cannot be built or the API rejects it.
	 *
	 * @return Booked_Shipment
	 */
	public function book( WC_Order $order, Delivery_Details $details, bool $is_return ): Booked_Shipment {
		$builder  = new Shipment_Builder( $order, new Order_Reader( $order ) );
		 ...
		 * Smart Send API. Return the (modified) Shipment.
		 *
		 * @since 9.0.0
		 *
		 * @param Shipment $shipment  The shipment to book.
		 * @param WC_Order             $order     The WooCommerce order.
		 * @param boolean              $is_return Whether the shipment is a return shipment.
		 *
		 * @return Shipment The shipment to book.
		 */
		$shipment = apply_filters( 'smart_send_booking_request', $shipment, $order, $is_return );

		$api = SS_SHIPPING_WC()->get_api_handle();

		$wire_shipment = $api->bookings()->from_shipment( $shipment );
		...
		try {
			...
			$exception = Booking_Exception::from_api_exception( $e );

			/*
			 * Action when the Smart Send API rejected a shipment. Fires
			 * right before the Booking_Exception is thrown
			 * (getMessage(), errors() for per-field validation errors,
			 * response_id(), getPrevious() for the API client exception).
			 *
			 * @since 9.0.0
			 *
			 * @param Booking_Exception $exception The failure.
			 * @param Shipment          $shipment  The shipment that was sent.
			 * @param WC_Order                      $order     The WooCommerce order.
			 */
		 ...
		 * @since 9.0.0
		 *
		 * @param Booked_Shipment $booked   The booked shipment (shipment id, tracking, parcels, documents, codes).
		 * @param Shipment        $shipment The shipment that was sent.
		 * @param WC_Order                    $order    The WooCommerce order.
		 */
	 ...
	 *
	 * @param object               $data      The v1 response data (Smart_Send\API\Response::data()).
	 * @param Shipment $shipment  The shipment representation that was booked.
	 * @param boolean              $is_return Whether the shipment is a return shipment.
	 *
	 * @return Booked_Shipment
	 */
	protected function map_v1_response( $data, Shipment $shipment, bool $is_return ): Booked_Shipment {
		$booked = new Booked_Shipment( isset( $data->shipment_id ) ? (string) $data->shipment_id : '' );
		...
		$booked
			...
			->set_state( Booked_Shipment::STATE_BOOKED )
		...
		if ( isset( $data->pdf ) ) {
			$document = new Shipment_Document(
				Shipment_Document::TYPE_LABEL,
				Shipment_Document::FORMAT_PDF,
			...
			// Shipment_Document::get_inline_content()).
