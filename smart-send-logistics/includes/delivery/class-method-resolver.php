 ...
 *
 * Extracted from Order_Meta::get_smart_send_method_id()
 ...
 * (Order_Meta) stays the single reader/writer of
...
class Method_Resolver {
	 ...
	 * @param integer|WC_Order $order Order (or order id).
	 *
	 * @throws Booking_Exception When the order's Smart Send shipping item has no return method configured.
	 *
	...
	public function resolve_return( $order ) {
		...
		if ( is_array( $resolved ) ) {
			if ( empty( $resolved['smart_send_return_method'] ) ) {
				// phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- the exception message is caught by Fulfillment_Service and recorded on the Fulfillment_Result as data, never echoed directly; escaping is not applicable here.
				throw new Booking_Exception( __( 'No return method set', 'smart-send-logistics' ) );
			}
	 ...
	 * moved verbatim from Order_Meta::get_smart_send_method_id()
