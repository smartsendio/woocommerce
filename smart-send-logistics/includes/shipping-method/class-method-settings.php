 ...
 * Method and validates the custom fields on save. The
 * rendering of the custom field types lives in
 * Method_Form_Renderer; the WooCommerce settings framework
...
class Method_Settings {
	...
	/**
	 * @param Method      $method  The shipping method instance.
	 * @param Method_Catalog $catalog Shipping method catalogue.
	 */
	public function __construct( Method $method, Method_Catalog $catalog ) {
		$this->method  = $method;
	...
	public function build_form_fields() {
		$log_path = Logger::get_log_url();
		// The formatter is stateless, so a fresh instance is safe here.
		$agents_address_format = ( new Pickup_Point_Formatter() )->get_format_options();

		return array(
			Settings::KEY_API_TOKEN               => array(
			...
			Settings::KEY_ORDER_STATUS            => array(
			...
			Settings::KEY_FREE_SHIPPING_METHOD    => array(
				'title'       => __(
			...
			Settings::KEY_INCLUDE_ORDER_COMMENT   => array(
				'title'    => __( 'Include order comment on label', 'smart-send-logistics' ),
				'default'  => 'no',
				'type'     => 'checkbox',
				'desc_tip' => false,
			),
			Settings::KEY_SAVE_LABELS_IN_UPLOADS  => array(
			...
			Settings::KEY_DROPDOWN_DISPLAY_FORMAT => array(
			...
			Settings::KEY_DEFAULT_SELECT_AGENT    => array(
			...
			Settings::KEY_SORT_METHODS_BY_COST    => array(
