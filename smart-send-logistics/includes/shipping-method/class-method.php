...
class Method extends WC_Shipping_Flat_Rate {
	 ...
	 * @var Method_Catalog
	 */
	protected Method_Catalog $catalog;
	 ...
	 * @var Method_Settings
	 */
	protected Method_Settings $settings_builder;

	/**
	 * Renderer for the custom settings field types.
	 *
	 * @var Method_Form_Renderer
	 */
	protected Method_Form_Renderer $form_renderer;

	/**
	 * Reporter for the availability outcomes (log + checkout debug bar).
	 *
	 * @var Availability_Reporter
	 */
	protected Availability_Reporter $availability_reporter;
	...
	public function __construct( $instance_id = 0 ) {
		...
		$this->settings_builder      = new Method_Settings( $this, $this->catalog );
		$this->form_renderer         = new Method_Form_Renderer( $this );
		$this->availability_reporter = new Availability_Reporter();
	 ...
	 *
	 * @see Method_Settings::build_form_fields()
	 *
	 * @return void
	 */
	 ...
	 *
	 * @see Method_Settings::build_instance_form_fields()
	 *
	 * @return void
	 */
	 ...
	 *
	 * @see Method_Settings::validate_title_field()
	 *
	 * @throws Exception
	 */
	 ...
	 *
	 * @see Method_Settings::validate_method_field()
	 *
	 * @throws Exception
	 */
	 ...
	 *
	 * @see Method_Settings::validate_cost_weight_field()
	 */
	 ...
	 *
	 * @see Method_Form_Renderer::generate_button_html()
	 */
	 ...
	 *
	 * @see Method_Form_Renderer::generate_selectopt_html()
	 */
	 ...
	 *
	 * @see Method_Form_Renderer::generate_cost_weight_html()
	 *
	 * @return string
	 */
	 ...
	 * Generate radio HTML.
	 *
	 * @see Method_Form_Renderer::generate_radio_html()
	 */
	...
	public function calculate_shipping( $package = array() ) {
		...
		$weight_table    = new Weight_Table( $this->get_option( 'cost_weight', array() ) );
		...
		if ( ! $weight_in_table ) {
			...
			// Weight_Table::rows_matching(); reported below).
		...
		// Built from the constant (never $this->id) so the hook name can't drift
		// out of sync with it if SS_SHIPPING_METHOD_ID's value ever changes - see #106.
		do_action( 'woocommerce_' . SS_SHIPPING_METHOD_ID . '_shipping_add_rate', $this, $rate ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- WooCommerce shipping-method extension hook; preserve its public name.
	}
	...
	public function is_available( $package ) {
		...
		if ( 'yes' == $this->get_instance_option( 'advanced_settings_enable' ) ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual -- pre-existing loose comparison; tightening is a behaviour change out of scope for the #43 move.
			...
			if ( ! empty( $display_shipping_class ) ) {
				...
				switch ( $display_shipping_class_opt ) {
					case Method_Settings::SHIPPING_CLASS_OPT_ALL:
						$is_available = $all_in_array;
						break;
					case Method_Settings::SHIPPING_CLASS_OPT_ONE:
						$is_available = $one_in_array;
						break;
					case Method_Settings::SHIPPING_CLASS_OPT_NALL:
						$is_available = ! $one_in_array;
						break;
					case Method_Settings::SHIPPING_CLASS_OPT_NONE:
						$is_available = ! $all_in_array;
						break;
				}
		...
		// Built from the constant (never $this->id) so the hook name can't drift
		// out of sync with it if SS_SHIPPING_METHOD_ID's value ever changes - see #106.
		return apply_filters( 'woocommerce_shipping_' . SS_SHIPPING_METHOD_ID . '_is_available', $is_available, $package, $this ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- WooCommerce shipping-method extension hook; preserve its public name.
	}
	...
	public function is_free_shipping( $package ) {
		...
		if ( in_array( $requires, array( Method_Settings::REQUIRES_COUPON, Method_Settings::REQUIRES_EITHER, Method_Settings::REQUIRES_BOTH ) ) ) { // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict -- pre-existing loose in_array; tightening is a behaviour change out of scope for the #43 move.
		...
		if ( in_array( $requires, array( Method_Settings::REQUIRES_MIN_AMOUNT, Method_Settings::REQUIRES_EITHER, Method_Settings::REQUIRES_BOTH ) ) ) { // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict -- pre-existing loose in_array; tightening is a behaviour change out of scope for the #43 move.
		...
		switch ( $requires ) {
			case Method_Settings::REQUIRES_MIN_AMOUNT:
				$is_available = $has_met_min_amount;
				break;
			case Method_Settings::REQUIRES_COUPON:
				$is_available = $has_coupon;
				break;
			case Method_Settings::REQUIRES_BOTH:
				$is_available = $has_met_min_amount && $has_coupon;
				break;
			case Method_Settings::REQUIRES_EITHER:
				$is_available = $has_met_min_amount || $has_coupon;
				break;
			case Method_Settings::REQUIRES_ENABLED:
				$is_available = true;
				break;
			case Method_Settings::REQUIRES_DISABLED:
			default:
				$is_available = false;
				break;
		}
		...
		return apply_filters(
			'woocommerce_shipping_' . SS_SHIPPING_METHOD_ID . '_is_free_shipping', // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- WooCommerce shipping-method extension hook; preserve its public name.
			$is_available,
			$package,
			$this
		);
	 ...
	 *
	 * @see Method_Catalog::get_shipping_method_name()
	 *
