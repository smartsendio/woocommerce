	...
	class SS_Shipping_WC_Order {
		...
		protected function define_constants() {
			SS_SHIPPING_WC()->define(
				...
				( SS_SHIPPING_WC()->get_demo_mode_setting() ? __(
					'DEMO MODE',
		...
		protected function init_hooks() {
			...
			if ( null !== $subs_version && version_compare( $subs_version, '2.0.0', '>=' ) ) {
				add_filter(
					...
					array( $this, 'woocommerce_subscriptions_renewal_order_meta_query' ),
					10
				);
			} else {
				add_filter(
									...
									array_push(
										...
										array(
											...
											. ( empty( $value['success']->woocommerce['return'] ) ?
												__(
													'Shipping label created by Smart Send',
													'smart-send-logistics'
												) : __(
													'Return label created by Smart Send',
													'smart-send-logistics'
												) )
											. ': ' . $this->get_ss_shipping_label_link(
												$value['success']->woocommerce['label_url'],
												! empty( $value['success']->woocommerce['return'] )
											),
											'type'    => 'success',
										)
							...
							array_push(
								...
								array(
									'message' => sprintf(
										...
										'The selected order did not include a Send Smart shipping method',
										'smart-send-logistics'
									),
									'type'    => 'error',
								)
		...
		public function render_smart_send_order_meta_box( $post_or_order_object ) {
			...
			if ( ! $ss_shipping_method_id ) {
				echo '<p>' . __(
					'Order placed with a shipping method that is not from the Smart Send plugin',
					'smart-send-logistics'
				) . '</p>';
			...
			echo '<p>' . sprintf(
				__( 'Weight: %0.2f kg', 'smart-send-logistics' ),
				$this->getOrderWeight( $order )
			) . '</p>';
			...
			wp_enqueue_script(
				...
				SS_SHIPPING_PLUGIN_DIR_URL . '/assets/js/ss-shipping-label.js',
				array(),
				SS_SHIPPING_VERSION
			);
		...
		protected function save_shipping_agent( $post_id, $doing_ajax, $ss_shipping_agent_no ) {
			...
			if ( ! empty( $ss_shipping_method_id ) ) {
				...
				if ( ! empty( $shipping_method_carrier ) && ! empty( $shipping_address['country'] ) ) {
					...
					if ( SS_SHIPPING_WC()->get_api_handle()->getAgentByAgentNo(
		...
		protected function get_formatted_order_note_with_label_and_tracking( $order_id, $api_shipment_response, $return ) {
			...
			$tracking_note = '<label>' . ( $return ? __(
				'Return shipping label',
				'smart-send-logistics'
			) : __( 'Shipping label', 'smart-send-logistics' ) ) . ': </label>'
		...
		private function get_bulk_actions() {
			return array(
				'ss_shipping_label_bulk'  => ( SS_SHIPPING_WC()->get_demo_mode_setting() ? __(
					'DEMO MODE',
					'smart-send-logistics'
				) . ': ' : '' ) . __(
					'Smart Send - Generate Labels',
					'smart-send-logistics'
				),
				'ss_shipping_return_bulk' => ( SS_SHIPPING_WC()->get_demo_mode_setting() ? __(
					'DEMO MODE',
					'smart-send-logistics'
				) . ': ' : '' ) . __(
					'Smart Send - Generate Return Labels',
					'smart-send-logistics'
				),
			);
		}
				...
				if ( count( $array_shipment_ids ) > 1 ) {
					...
					if ( SS_SHIPPING_WC()->get_api_handle()->isSuccessful() ) {
						...
						array_push(
							...
							array(
								'message' => __(
									'Error combining shipping labels:',
									'smart-send-logistics'
