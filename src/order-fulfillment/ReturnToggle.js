 ...
 * one) the checkbox stays usable and, once ticked, a return method select
 * (state.methods.return) is required for the combined outbound + return
 * run.
 */
		...
		<div className="smart-send-fulfillment__row" data-ss-section="return">
			...
			{ ! configuredMethod && (
				<p className="description" data-ss-hint="no_return_method">
					{ __( 'No return method configured on the shipping method - choose one below, or set one under WooCommerce → Shipping → the zone method.', 'smart-send-logistics' ) }
				</p>
			) }
			{ ! configuredMethod && checked && (
