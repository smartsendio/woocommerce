 ...
 */
function asTestConnection( btn_id ) {
	...
	var data = {
		'action': 'test_as_connection',
		'test_con_nonce': as_test_con_obj.test_con_nonce
	};
	...
	jQuery.post(as_test_con_obj.ajax_url, data, function(response) {
		btn.attr("disabled", false);
		btn.text( response.button_txt );
		loaderContainer.remove();

		if ( response.connection_success ) {
			var test_connection_class = 'as_connection_succeeded';
			var test_connection_text = response.connection_success;
		} else {
			var test_connection_class = 'as_connection_error';
