...
Each screenshot owns fresh fixtures, mocked Smart Send responses and independent orders; email and external requests are blocked during capture. Capture the **complete viewport** by default, preserving the real WordPress/WooCommerce surroundings so merchants can recognize where to work. Scroll the relevant section into view and increase the viewport height when needed for readable controls. Element crops are supported only for rare, deliberate exceptions; do not crop ordinary order-panel captures. `highlight_element()` adds removable Dumbledore-inspired overlays and Smart Send green borders (`rgb(46, 92, 74)`) without rewriting control labels or changing layout. Use `overlay: false` when surrounding settings should remain equally visible, and `clear_highlights()` before another action. English and Danish runs use real installed language packs, including the administrator locale. Missing Smart Send translations are recorded in the bundle; untranslated controls must not be relabelled for a screenshot.

The required images and guide destinations are listed in `tests/Docs/catalog.json`. Add a catalog entry with each scenario. Keep ordinary outbound booking separate from returns; use **Colli shipments / Kolli-forsendelser** only in the advanced section of the booking guide. Customs screenshots show the product **Shipping** tab. Do not capture hypothetical upcoming bulk UI. Fixture order dates are fixed; booking and native order-note timestamps show the actual run time.
...
The release-tool regression check runs locally with `bash tests/Release/svn-deploy.sh` (requires `svn` and `svnadmin`). It uses disposable local repositories to verify trunk and tag contents; it never publishes to WordPress.org.
