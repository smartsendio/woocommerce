...
1. **Booking domain** in `includes/booking/` — everything that turns an order into a booked label:
   ...
   - `SS_Shipping_Booking_Service` — the order-level booking orchestrator, stateless (constructed once, `WC_Order` passed per call): builds the shipment via the builder, hands it to `Smartsend\Resources\BookingResource`, wraps the outcome in a `SS_Shipping_Booking`. `SS_Shipping_Booking_Exception` (in `booking/exceptions/`) is caught here and converted into a failed booking; it is thrown by `SS_Shipping_Method_Resolver::resolve_return()` when no return method is configured — the one deliberate delivery → booking reference, pinned by tests.
