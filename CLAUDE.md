...
1. **Admin classes** in `admin/`:
   ...
   - The admin order integration is no longer a single facade class - the `SS_Shipping_WC` singleton constructs and owns each component directly, reachable via typed accessors: `SS_Shipping_Order_Meta` (order meta access: method id, pick-up point agent, parcel split, shipment ids — `order_meta()`), `SS_Shipping_Order_Meta_Box` (order screen meta box — `meta_box()`), `SS_Shipping_Fulfillment_Service` (the label fulfillment workflow — `fulfillment()`), `SS_Shipping_Label_Creator` (the AJAX `wp_ajax_ss_shipping_generate_label` controller: nonce check, request parsing, then delegates to the fulfillment service), `SS_Shipping_Order_Bulk_Actions` (bulk label actions on the Orders screen — temporarily single-order-only: selecting more than one order errors without processing, pending the Phase 7 bulk rebuild, #116 — `bulk_actions()`), `SS_Shipping_Admin_Notices` (flash notices store — `admin_notices()`). Supports both legacy post-based orders and HPOS (the plugin declares HPOS compatibility). WooCommerce Subscriptions integration lives on its own `SS_Shipping_Subscriptions_Compat` (in `includes/`, since the WC_Subscriptions plugin dependency is optional per the existence-guard policy), not on the singleton.
