...
it('carries one customer text per non-found status and none for found', function () {
    ...
    expect($options->pickup_point_status_message(SS_Shipping_Checkout_Options::STATUS_FOUND))->toBeNull()
        ->and($options->pickup_point_status_message(SS_Shipping_Checkout_Options::STATUS_ADDRESS_INCOMPLETE))
        ->toBe('Enter your shipping address to see available pickup points.')
        ->and($options->pickup_point_status_message(SS_Shipping_Checkout_Options::STATUS_NOT_CONNECTED))
        ->toBe('Connect the Smart Send plugin to enable pickup points.')
        ->and($options->pickup_point_status_message(SS_Shipping_Checkout_Options::STATUS_AUTH_FAILED))
        ->toBe('The shop is not correctly connected with Smart Send.')
        ->and($options->pickup_point_status_message(SS_Shipping_Checkout_Options::STATUS_ACCESS_DENIED))
        ->toBe('The shop does not have access to pickup points.')
        ->and($options->pickup_point_status_message(SS_Shipping_Checkout_Options::STATUS_NONE_FOUND))
        ...
        ->and($options->pickup_point_status_message(SS_Shipping_Checkout_Options::STATUS_LOOKUP_FAILED))
...
it('styles only the shop-side connection problems as errors', function () {
    ...
    expect($options->is_pickup_point_error_status(SS_Shipping_Checkout_Options::STATUS_NOT_CONNECTED))->toBeTrue()
        ->and($options->is_pickup_point_error_status(SS_Shipping_Checkout_Options::STATUS_AUTH_FAILED))->toBeTrue()
        ->and($options->is_pickup_point_error_status(SS_Shipping_Checkout_Options::STATUS_ACCESS_DENIED))->toBeTrue()
        ->and($options->is_pickup_point_error_status(SS_Shipping_Checkout_Options::STATUS_ADDRESS_INCOMPLETE))->toBeFalse()
        ->and($options->is_pickup_point_error_status(SS_Shipping_Checkout_Options::STATUS_NONE_FOUND))->toBeFalse()
        ->and($options->is_pickup_point_error_status(SS_Shipping_Checkout_Options::STATUS_LOOKUP_FAILED))->toBeFalse()
        ->and($options->is_pickup_point_error_status(SS_Shipping_Checkout_Options::STATUS_FOUND))->toBeFalse();
});
